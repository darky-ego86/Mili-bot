  ...
  run: async (client, message, args) => {
    ...
    if (!user)
      return message.channel.send({
        ...
        new TextDisplayBuilder().setContent("Please mention a valid user or provide a valid ID."),
        ],
      });
      ...
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(`❌ <@${user.id}> does not have NoPrefix access.`),
        ],
      });
    ...
    if (remaining !== null && remaining <= 0) {
      ...
      return message.channel.send({
        embeds: [
          new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`❌ <@${user.id}>'s NoPrefix access has expired.`)
      ),
        ],
      });
    }
