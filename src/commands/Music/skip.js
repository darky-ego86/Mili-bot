  ...
  run: async (client, message, args, prefix, player) => {
    ...
    if (!player) {
      const embed = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`${cross} | No song is currently playing.\nStart playing something to skip.`)
      );
    ...
    if (!args[0]) {
      ...
      await player.stop();
      const embed = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`${tick} | Skipped the current song.\nPlaying the next one in the queue.`)
      );
      return message.reply({ components: [embed], flags: MessageFlags.IsComponentsV2 });
    }
    ...
    if (amount > player.queue.length) {
      return message.reply({
        ...
        new TextDisplayBuilder().setContent(`${cross} | There aren't that many songs in the queue to skip.`)
      ),
        ],
      });
    }
