  ...
  run: async (client, message, args) => {
    ...
    if (!newPrefix) {
      return message.channel.send({
        ...
        new TextDisplayBuilder().setContent(`${cross} | Please provide a new prefix.`)
      )
        ],
    ...
    if (newPrefix.length > 5) {
      return message.channel.send({
        embeds: [
          new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`${cross} | Prefix too long. Maximum 5 characters allowed.`)
      )
        ],
        allowedMentions: { repliedUser: false }
      });
    ...
    collector.on("collect", async (interaction) => {
      ...
      if (interaction.customId === "confirm_prefix") {
        ...
        await interaction.update({
          embeds: [
            new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`${cross} | Prefix change cancelled.`)
      )
          ],
          components: []
        });
    ...
    collector.on("end", async (collected) => {
      if (collected.size === 0) {
        await msg.edit({
          embeds: [
            new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`${cross} | No response received. Prefix change request timed out.`)
      )
          ],
          components: []
        });
