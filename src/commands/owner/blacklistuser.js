  ...
  run: async (client, message, args, prefix) => {
    ...
    try {
      if (!args[0]) {
        ...
        new TextDisplayBuilder().setContent(`Use ${prefix}bluser add/remove/list [userId]`)
      );
      ...
      if (args[0].toLowerCase() === "add") {
        if (!userId || isNaN(userId)) {
          const embed = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent("Please provide a valid user ID to blacklist.");
          return message.channel.send({ components: [embed], flags: MessageFlags.IsComponentsV2 });
        }
        ...
        const embed = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`User <@${userId}> has been blacklisted.`)
      );
        return message.channel.send({ components: [embed], flags: MessageFlags.IsComponentsV2 });
        ...
        if (!userId || isNaN(userId)) {
          const embed = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent("Please provide a valid user ID to remove from blacklist.");
          return message.channel.send({ components: [embed], flags: MessageFlags.IsComponentsV2 });
        }
        ...
        const embed = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`User <@${userId}> has been removed from the blacklist.`)
      );
        return message.channel.send({ components: [embed], flags: MessageFlags.IsComponentsV2 });
