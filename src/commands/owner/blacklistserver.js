  ...
  module.exports = {
    ...
    run: async (client, message, args, prefix) => {
      ...
      try {
        ...
        new TextDisplayBuilder().setContent(`Use ${prefix}blserver add/remove/list <serverId>`)
      );
        ...
        if (args[0].toLowerCase() === "add") {
          if (data) {
            const embed = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent("Server already blacklisted");
            return message.channel.send({ components: [embed], flags: MessageFlags.IsComponentsV2 });
          }
        ...
        if (args[0].toLowerCase() === "remove") {
          if (!args[1]) {
            const embed = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent("Please provide a valid server ID to remove");
            return message.channel.send({ components: [embed], flags: MessageFlags.IsComponentsV2 });
          }
