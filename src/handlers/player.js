  ...
  client.manager.on("queueEnd", async (player) => {
    try {
      ...
      const embed = new EmbedBuilder()
        .setAuthor({
          name: "Tedex Queue Finished",
          iconURL: client.user.displayAvatarURL(),
        })
        .setColor(client.color)
        .setDescription(
          `> The music queue has now reached its end.\n\n` +
          `> Thank you for vibing with **Mili**! 💜\n\n` +
          `> Invite Mili to more servers or join our support server to stay updated:\n\n` +
          `> [Invite Mili](https://discord.com/oauth2/authorize?client_id=1390965840437968896&permissions=8&integration_type=0&scope=bot)\n` +
          `> [Join Support Server](https://discord.gg/zenkai-headquarters-6-1363548442449674482)`
        )
        .setFooter({
